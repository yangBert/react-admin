...
const changeAddModalvisibleAction = addModalvisible => ({
  type: types.PRODUCT_TYPE_CHANGE_ADD_MODAL_VISIBLE,
  addModalvisible,
})

//改变修改弹出层显示和隐藏
const changeEditModalvisibleAction = editModalvisible => ({
  type: types.PRODUCT_TYPE_CHANGE_EDIT_MODAL_VISIBLE,
  editModalvisible,
})

//初始化要编辑的某一行数据
const changeEditRecordAction = editRecord => ({
  type: types.PRODUCT_TYPE_CHANGE_EDIT_RECORD,
  editRecord
})
...
const initMenuAction = (list) => ({
  type: types.PRODUCT_TYPE_INIT_MENU_LIST,
  list,
})

//新增弹出层提交按钮loading状态
const changeaddConfirmLoadingAction = addConfirmLoading => ({
  type: types.PRODUCT_TYPE_CHANGE_ADD_CONFIRM_LOADING,
  addConfirmLoading,
})

//改变编辑弹出层提交按钮loading状态
const changeEditConfirmLoadingAction = editConfirmLoading => ({
  type: types.PRODUCT_TYPE_CHANGE_EDIT_CONFIRM_LOADING,
  editConfirmLoading,
})
